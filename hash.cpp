...
class Diretorio {
  public:
    ...
    class Bucket {
      public:
        ...
        bool estaCheio() {  
          if(size >= SIZEBUCKET) return true; //verificando se o bucket está cheio
          return false;
        }
    ...
    Diretorio(int pGlobal) { 
		...
		for(int i = 0; i < qtdPonteiros; i++) { // inicializando buckets
	...
	int* inicializar_vetor(int size) {
		...
		for(int i = 0; i < size; i++) newVetor[i] = 0;
		return newVetor;
	}
  ...
  int* toBinary(int value, bool getLsbs, int profundidade) { 
	...
	if(bitSize < profundidade) bitSize = profundidade;
	
	if(getLsbs)
		for(i = 0; i < profundidade; i++) lsb[(profundidade-1) - i] = bits[i];
	else
		for(i = bitSize-1; i >= 0; i--) lsb[(bitSize-1) - i] = bits[i];
   ...
   Vinho createVinho(int id, int ano_colheita, string rotulo, string tipo) {
	...
	int searchFunctionHash(int* lsbVinho) {
		...
		for(int i = 0; i < qtdPonteiros; i++) {
			...
			for(int j = 0; j < profGlobal; j++) {
				if(hashs[i].lsb[j] != lsbVinho[j]) { encontrado = false; continue; }
			}
	...
	void percorrerBuckets() {
			for(int i = 0; i < qtdPonteiros; i++) {
				for(int k = 0; k < profGlobal; k++) cout << hashs[i].lsb[k];
   ...
   void insert(int id, int ano_colheita, string rotulo, string tipo) {
	...
	if(bucketEncontrado->estaCheio()) {
		if(bucketEncontrado->profLocal < profGlobal) dividir_bucket(posHash);
		else {
